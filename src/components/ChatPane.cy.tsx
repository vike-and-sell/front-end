...
import { mockChatPaneItems } from "../utils/mockChatPanelItems";
...
describe("<ChatPane />", () => {
  ...
  it("Filter chats based on search inputer", () => {
    ...
    const expectedChats = mockChatPaneItems.filter(
      (item) =>
    ...
    cy.get('[data-cy="chat-pane-item"]').should(
      "have.length",
      expectedChats.length
    );

    expectedChats.forEach((item: ChatType, index) => {
      cy.get('[data-cy="chat-pane-item"]')
        .eq(index)
        .contains(item.interlocutor.username);
    });
  });
